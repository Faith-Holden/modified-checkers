 ...
 * moves, the game ends.
 */
public class CheckersUpdate extends Application {
    private static final String DATA_FILE_NAME = "checkers_save_file.txt";
    File saveDirectory = new File("src\\resources\\files");
    ...
    public void start(Stage stage) {
        ...
        resignButton = new Button("Resign");//added by HFH
        loadGameButton = new Button("Load Game");//added by HFH
        ...
        loadGameButton.setOnAction(e-> board.doLoadGame());//added by HFH
        ...
        loadGameButton.relocate(370, 40);
        newGameButton.relocate(370, 120);
        saveButton.relocate(370, 200);
        resignButton.relocate(370, 280);
        ...
        root.getChildren().addAll(board, newGameButton, saveButton, loadGameButton,resignButton, message);
    ...
    private class CheckersBoard extends Canvas {
        ...
        void doNewGame() {
            ...
            board.setUpNewGame();   // Set up the pieces.
        ...
        //added by HFH
        void doSaveGame(){
            File saveFile = new File(saveDirectory, DATA_FILE_NAME );
            System.out.println("Saving checkers game to file " +
            ...
            for(int i = 0; i<8; i++){
                ...
                for(int j=0; j<8; j++){
                    out.println("       <col num='" + j + "' player='" + saveData[i][j] + "'/>");
                }
                out.println("   </row>");
            }
            out.println("</rows>");
            out.println("<current_player player='" + currentPlayer + "'/>");
        ...
        //added by HFH
        void doLoadGame(){
            board = loadBoard(); // Set up game with data read from file."
        ...
        //added by HFH
        CheckersData loadBoard(){
            File saveFile = new File(saveDirectory, DATA_FILE_NAME );
            return new CheckersData(saveFile);
        }

        //added by HFH
        int loadCurrentPlayer(){
            File userHomeDirectory = new File( System.getProperty("user.home") );
            File saveFile = new File(userHomeDirectory, DATA_FILE_NAME );
            Document checkersSaveDoc;
            try{
                DocumentBuilder docReader
                        = DocumentBuilderFactory.newInstance().newDocumentBuilder();
                checkersSaveDoc = docReader.parse(saveFile);
            }catch (Exception e){
                System.out.println("Error in checkers save file.");
                System.out.println("File name:  " + saveFile.getAbsolutePath());
                System.out.println("This program cannot continue.");
                return -1;
            }
            NodeList currentPlayers = checkersSaveDoc.getElementsByTagName("current_player");
            Element currentPlayer = (Element) currentPlayers.item(0);
            return Integer.parseInt(currentPlayer.getAttribute("player"));
        }
    ...
    private static class CheckersData {
        ...
        CheckersData(File saveFile){
            board = parseSaveFile(saveFile);
        }
        ...
        int[][] parseSaveFile(File saveFile){
            int[][]savedGame = new int[8][8];
            Document checkersSaveDoc;
            try{
                DocumentBuilder docReader
                        = DocumentBuilderFactory.newInstance().newDocumentBuilder();
            ...
            NodeList rowList = checkersSaveDoc.getElementsByTagName("row");
            for(int i = 0; i<rowList.getLength(); i++){
                if(rowList.item(i) instanceof Element){
                    Element currentRow = (Element)rowList.item(i);
                    int rowNum = Integer.parseInt(currentRow.getAttribute("num"));
                    NodeList colList = currentRow.getElementsByTagName("col");
                    for(int j = 0; j<colList.getLength(); j++){
                        if(rowList.item(i) instanceof Element){
                            Element currentCol = (Element)colList.item(j);
                            int colNum = Integer.parseInt(currentCol.getAttribute("num"));
                            int squareValue = Integer.parseInt(currentCol.getAttribute("player"));
                            savedGame[rowNum][colNum]=squareValue;
                        }
                    }
                }
            }
